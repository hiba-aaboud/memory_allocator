...
void coalesce_blocks(mem_pool_t *pool, mem_std_free_block_t *block) {
    ...
    if ((char *)next_block < (char *)pool->end_addr && is_block_free(&next_block->header)) {
        ...
        // Ensure footer is within bounds
 ...
 if (pool->first_free ==  block->next) {
                pool->first_free = block;
            }
        printf("Coalescing with next block\n");
        printf("Block start: %p, Block size: %zu\n", block, get_block_size(&block->header));
        printf("Next block start: %p, Next block size: %zu\n", next_block, next_block_size);
        printf("New coalesced size: %zu\n", get_block_size(&block->header));
    }
    ...
    if (prev_block && is_block_free(&prev_block->header)) {
        ...
        *footer = prev_block->header;
        ...
        if (block->next) {
         ...
         if (pool->first_free == block) {
                pool->first_free = prev_block;
            }

        printf("Coalescing with previous block\n");
        printf("Prev block start: %p, Prev block size: %zu\n", prev_block, prev_block_size);
        printf("Block start: %p, Block size: %zu\n", block, get_block_size(&block->header));
        printf("New coalesced size: %zu\n", get_block_size(&prev_block->header));
    }
}
...
//         if (is_block_free(&current->header) && block_size >= size) {
...
//         split_free_block(pool, best_fit, size);
